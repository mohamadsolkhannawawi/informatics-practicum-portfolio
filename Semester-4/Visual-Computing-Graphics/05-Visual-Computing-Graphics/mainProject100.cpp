...
void display(void) {
    ...
    // Pergelangan tangan (Telapak tangan diperpendek)
	glTranslatef(1.0, 0.0, 0.0);
	glRotatef((GLfloat) pergelangan, 0.0, 0.0, 1.0);
	glTranslatef(0.25, 0.0, 0.0); // Sesuaikan posisi agar tetap tersambung dengan lengan
	glPushMatrix();
	glScalef(0.5, 0.3, 0.7); // Telapak lebih pendek, sekitar 1/4 panjang lengan
	glutWireCube(1.0);
	glPopMatrix();
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case 'P': pergelangan = (pergelangan - 5) % 360; break;
        case '1': ibu_jari = (ibu_jari + 5) % 360; break;
        case '!': ibu_jari = (ibu_jari - 5) % 360; break;
        case '2': telunjuk = (telunjuk + 5) % 360; break;
        case '@': telunjuk = (telunjuk - 5) % 360; break;
        case '3': jari_tengah = (jari_tengah + 5) % 360; break;
        case '#': jari_tengah = (jari_tengah - 5) % 360; break;
        case '4': jari_manis = (jari_manis + 5) % 360; break;
        case '$': jari_manis = (jari_manis - 5) % 360; break;
        case '5': kelingking = (kelingking + 5) % 360; break;
