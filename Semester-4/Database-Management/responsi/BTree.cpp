...
void BTree<T>::remove(int key, Node<T>* curr) { //
    ...
    if (curr && curr->parent) {
        bool parentIsRoot = (curr->parent == root);
        int parentMinKeys = parentIsRoot ? 1 : minCapacity;
        if (curr->parent->size < (parentIsRoot && curr->parent->children.empty() ? 0 : parentMinKeys) ) {
...
bool BTree<T>::empty() const {
    return root->size == 0 && root->type == NodeType::NODE_LEAF;;
}
