...
CREATE FUNCTION fn_GetTransactionCountByType (@type VARCHAR(10)) 
RETURNS INT 
AS 
BEGIN 
	-- Kamus Lokal 
...
-- Penggunaan
SELECT dbo.fn_GetTransactionCountByType('Transfer') 
AS TotalTransaksi;
-- Hapus Fungsi
DROP FUNCTION fn_GetTransactionCountByType
-- Pembuktian
...
BEGIN
    ...
    DECLARE @financialSummary DECIMAL(18, 2);
    ...
    SELECT @totalLoans = ISNULL(SUM(loan_amount), 0)
    FROM loans
    WHERE customer_id = @customer_id AND status = 'active'; 

    SET @financialSummary = @totalBalance + @totalLoans;
